...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/05/02 22:18:23 by kong              #+#    #+#             */
/*   Updated: 2026/05/05 21:13:25 by kong             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	_nbr_only(t_coord *dest, char *token)
{
	t_coord	new_coord;
...
static int	_nbr_with_color(t_coord *dest, char *token, char *color_ptr)
{
	t_coord	new_coord;
	int		color;
	char	*nbr_ptr;
	...
	if (!nbr_ptr || !is_valid_int(nbr_ptr))
	{
		free(nbr_ptr);
		return (-1);
	}
...
static int	_parse_coord_node(t_coord *dest, char *token, t_map *map)
{
	...
	if (!color_ptr)
	{
		if (_nbr_only(dest, token) == -1)
			return (-1);
	}
	else
	{
		if (_nbr_with_color(dest, token, color_ptr) == -1)
			return (-1);
	}
	_update_map_z(map, *dest);
	return (1);
}
...
t_coord	*parse_coord_arr(char *str, t_map *map)
{
	...
	if (map->n_col != 0 && map->n_col != ft_count_list(tokens_lst))
		return (perrmsg("FdF: Invalid map"), freelst((void **)tokens_lst),
			NULL);
	...
	while (tokens_lst[i])
	{
		...
		if (_parse_coord_node(&(coord_arr[i]), tokens_lst[i], map) == -1)
			return (perrmsg("FdF: Invalid map"), freelst((void **)tokens_lst),
				free(coord_arr), NULL);
		i++;
	}
