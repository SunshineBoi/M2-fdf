...
/*   Updated: 2026/04/30 17:41:09 by kong             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	img_obj->img_ptr = mlx_new_image(mlx_obj->conn_ptr, vp_obj->win_width, vp_obj->win_height);
	if (!img_obj->img_ptr)
		return (print_errmsg("FdF: MLX image failure."), -1);
	img_obj->pixel_ptr = mlx_get_data_addr(img_obj->img_ptr, &img_obj->bpp, &img_obj->spl, &img_obj->endian);
	if (!img_obj->pixel_ptr)
		return (mlx_destroy_image(mlx_obj->conn_ptr, img_obj->img_ptr), print_errmsg("FdF: MLX image failure."), -1);
	img_obj->is_dirty = 0;
	return (0);
}
...
int	rerender_img(t_data *data)
{
	...
	if (create_mlx_img(data->mlx_obj, data->img_obj, data->vp_obj) == -1)
		return (-1);
...
t_image	*init_img(t_mlx *mlx_obj, t_viewpoint *vp_obj)
{
	...
	if (create_mlx_img(mlx_obj, new_img, vp_obj) == -1)
...
void	draw_line(t_data *data, t_coord start, t_coord end)
{
	...
	if (dx == 0 && dy == 0)
	{
		_color_pixel(data, start.x, start.y, start.color);
		return ;
	}
	int	err = 0;
	if (dx >= dy)
	...
	while (step < total_steps)
	{
		...
		_color_pixel(data, *screen_x, *screen_y, color);
		if (err >= boundary)
		{
			minor += minor_dir;
			err -= reset;
...
int	render_img(t_data *data)
{
	...
	while (row < row_size)
	{
		...
		while (col < col_size)
		{
			...
			// 3. spin rotation
			x_trans = (x_world * cos(data->vp_obj->spin_deg * DEG_TO_RAD)) -
				(y_world * sin(data->vp_obj->spin_deg * DEG_TO_RAD));
			y_trans = (x_world * sin(data->vp_obj->spin_deg * DEG_TO_RAD)) +
				(y_world * cos(data->vp_obj->spin_deg * DEG_TO_RAD));
			// 4. pitch rotation
			y_trans = (y_trans * cos(data->vp_obj->pitch_deg * DEG_TO_RAD)) -
				(z_world * sin(data->vp_obj->pitch_deg * DEG_TO_RAD));
			// 5. zoom factor - it determines the final spacing between points.
			x_trans *= data->vp_obj->f_zoom;
			y_trans *= data->vp_obj->f_zoom;
			// 6. convert to pixel coordinate
			x_pixel = x_trans + data->vp_obj->x_win_anchor;
			y_pixel = y_trans + data->vp_obj->y_win_anchor;
			// create a new point
			point.x = x_pixel;
			point.y = y_pixel;
